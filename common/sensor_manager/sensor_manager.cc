...
#include "common/io/io_util.h"
#include "lib/config_manager/config_manager.h"
#include "proto/sensor_meta_schema.pb.h"
...
bool SensorManager::Init() {
  ...
  auto AddSensorInfo = [this](const SensorMeta& sensor_meta_proto) {
    ...
    if (this->IsCamera(sensor_info.type)) {
      ...
      const std::string intrinsic_path = cyber::common::GetAbsolutePath(
          lib::ConfigManager::Instance()->work_root(), intrinsic_file);

      if (!LoadBrownCameraIntrinsic(intrinsic_path, distort_model.get())) {
